...
cursor = con.execute('''SELECT Bike_Id, COUNT(Rental_Id) AS count, End_Date, Start_Date FROM journeys GROUP BY Bike_Id''')
...
bike_df['Num_Journeys'] = bike_df['count']/total_duration
...
axes.hist(bike_df['Num_Journeys'], 50, alpha=0.8, edgecolor='k', color='red')
