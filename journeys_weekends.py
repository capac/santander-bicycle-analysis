...
query = '''SELECT Day, Bike_Id, ROUND(AVG(Num_Rides), 2) AS Average FROM (SELECT strftime("%Y-%m-%d", End_Date) AS Day, Bike_Id, COUNT(Rental_Id) AS Num_Rides, SUM(Duration) AS Tot_Time_Rides FROM Journeys WHERE strftime("%w", End_Date) IN ("0", "6") GROUP BY Day, Bike_Id HAVING Tot_Time_Rides > 0) GROUP BY Bike_Id'''
...
axes.set_xlabel('Average number of journeys per bike on weekends')
axes.set_ylabel('Counts')
axes.set_title('Histogram of average number of journeys per bike on weekends')
plt.grid(linestyle=':')
