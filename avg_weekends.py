...
query = ''' SELECT Hour, ROUND(AVG(Num_Rides), 2) AS Avg_Rides, ROUND(AVG(Num_Bikes), 2) AS Avg_Bikes FROM (SELECT strftime("%Y-%m-%d", End_Date) AS Day, strftime("%H", End_Date) AS Hour, COUNT(Rental_Id) AS Num_Rides, COUNT(DISTINCT(Bike_Id)) AS Num_Bikes FROM Journeys WHERE strftime("%w", End_Date) IN ("0", "6") GROUP BY Day, Hour HAVING Duration > 0) GROUP BY Hour'''
