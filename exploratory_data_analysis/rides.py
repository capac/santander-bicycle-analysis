...
for dt, lb, sp in zip([(2019, 6, 10), (2019, 12, 25)], ['Rainy day', 'Christmas'], [(7.5, 1.2), (7.5, 3.2)]):
    axes.vlines(datetime(*dt), yaxis_limits[0], yaxis_limits[1], linestyle=':', color='k')
    axes.annotate(lb, xy=(datetime(*dt)-timedelta(sp[0]), axes.yaxis.get_data_interval()
                                     [0] + sp[1]), ha='left', va='bottom', rotation=90, size=9, color='k')
