...
for year in years:
    for value in values:
        ...
        df = pd.DataFrame(query_results[1], columns=[
                          x[0] for x in query_results[0].description])
        # print(df.head())
...
results_df['Month'] = results_df['Month'].replace({'01': 'Jan', '02': 'Feb',
                                                   '03': 'Mar', '04': 'Apr',
                                                   '05': 'May', '06': 'Jun',
                                                   '07': 'Jul', '08': 'Aug',
                                                   '09': 'Sep', '10': 'Oct',
...
columns = [['Weekends_2019', 'Weekdays_2019'],
           ['Weekends_2020', 'Weekdays_2020']]
colors = [['forestgreen', 'limegreen'], ['royalblue', 'dodgerblue']]
width = 0.35
widths = [-width, width]
df = results_df.iloc[0:5].copy()
df.loc[:, [c for col in columns for c in col]] = df[[
    c for col in columns for c in col]].divide(1e5)
# print(df)
for col, color, wd in zip(columns, colors, widths):
    ax.bar(np.arange(5) + wd/2, df[col[0]], width,
           label=col[0].replace('_', ' '), color=color[0], edgecolor='k')
    ax.bar(np.arange(5) + wd/2, df[col[1]], width, label=col[1].replace(
        '_', ' '), color=color[1], edgecolor='k', bottom=df[col[0]])
